...
import ml.shifu.core.container.obj.PerformanceResult;
...
public class PerformanceEvaluator {
    ...
    public void review() throws IOException {
        ...
        BufferedReader reader = ShifuFileUtils.getReader(
    ...
    public void review(List<ConfusionMatrixObject> matrixList)
            throws IOException {
        PathFinder pathFinder = new PathFinder(modelConfig);

        // bucketing
        PerformanceResult result = bucketing(matrixList,
                evalConfig.getPerformanceBucketNum(),
                evalConfig.getDataSet().getWeightColumnName() != null);

        Writer writer = null;
        try {
            writer = ShifuFileUtils.getWriter(
                    pathFinder.getEvalPerformancePath(evalConfig, evalConfig.getDataSet().getSource()),
                    evalConfig.getDataSet().getSource());
    ...
    private PerformanceObject setPerformanceObject(ConfusionMatrixObject confMatObject) {
        ...
        po.liftUnit = confMatObject.getTp()
                / ((confMatObject.getTp() + confMatObject.getFp()) * (confMatObject.getTp() + confMatObject.getFn())
                / confMatObject.getTotal());
    ...
    public PerformanceResult bucketing(List<ConfusionMatrixObject> results, int numBucket, boolean isWeight) {

        List<PerformanceObject> FPRList = new ArrayList<PerformanceObject>(
                numBucket + 1);
        List<PerformanceObject> catchRateList = new ArrayList<PerformanceObject>(
                numBucket + 1);
        List<PerformanceObject> gainList = new ArrayList<PerformanceObject>(
                numBucket + 1);

        List<PerformanceObject> FPRWeightList = new ArrayList<PerformanceObject>(
                numBucket + 1);
        List<PerformanceObject> catchRateWeightList = new ArrayList<PerformanceObject>(
                numBucket + 1);
        List<PerformanceObject> gainWeightList = new ArrayList<PerformanceObject>(
                numBucket + 1);
        ...
        PerformanceObject po = setPerformanceObject(results.get(0));
        ...
        for (int i = 1; i < results.size(); i++) {
            ...
            ConfusionMatrixObject object = results.get(i);
            ...
            // prevent 99%
            if ((double) (i + 1) / results.size() >= gainBin * binCapacity) {
                po.binNum = gainBin++;
                gainList.add(po);
            }
            ...
            if ((object.getWeightedTp() + object.getWeightedFp() + 1)
                    / object.getWeightedTotal() >= gainWeightBin * binCapacity) {
    ...
    private void logResult(List<PerformanceObject> list, String info) {
        ...
        log.info(String.format(formatString, "ActionRate",
                "WeightedActionRate", "Recall", "WeightedRecall", "Precision", "WeightedPrecision",
                "FPR", "WeightedFRP", "BinLowestScore"));

        for (PerformanceObject po : list) {
            log.info(String.format(formatString, df.format(po.actionRate),
                    df.format(po.weightedActionRate), df.format(po.recall),
                    df.format(po.weightedRecall), df.format(po.precision),
                    df.format(po.weightedPrecision), df.format(po.fpr),
                    df.format(po.weightedFpr), po.binLowestScore));
        }

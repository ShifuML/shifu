...
public class AllInclusiveTransformationExecutor {

    private static Logger log = LoggerFactory.getLogger(AllInclusiveTransformationExecutor.class);
  /*
    public Object transform(DerivedField derivedField, Object origin) {
        ...
        if (expression instanceof NormContinuous) {
           ...
           */
    public Map<String, Object> transform(PMML pmml, Model model, Map<String, Object> rawDataMap) {

        Map<String, Object> result = new LinkedHashMap<String, Object>();

        for (DataField dataField : pmml.getDataDictionary().getDataFields()) {
            result.put(dataField.toString(), rawDataMap.get(dataField.toString()));
        }

        if (model.getLocalTransformations() != null) {
            for (DerivedField derivedField : model.getLocalTransformations().getDerivedFields()) {
                String fieldNameString = derivedField.toString();
                //if (result.containsKey(fieldNameString)) {
                //    log.warn("");
                //} else {
                    result.put(derivedField.toString(), transform(derivedField, rawDataMap));
                //}
            }
        }

        // TODO: add TransformationDictionary

...
import ml.shifu.core.core.processor.*;
import ml.shifu.core.core.processor.EvalModelProcessor.EvalStep;
import ml.shifu.core.core.processor.ManageModelProcessor.ModelAction;
...
public class ShifuCLI {
            ...
            if (args[0].equals(NEW) && args.length >= 2 && StringUtils.isNotEmpty(args[1]) && args[1].trim().matches("^[^-~].*")) {
                ...
                if (args[0].equals(MODELSET_CMD_CP) && args.length >= 3 && StringUtils.isNotEmpty(args[1])
                    ...
                    if (cmd.getOptions() == null || cmd.getOptions().length == 0) {
                        ...
                        if (status == 0) {
                            log.info("ModelSet initilization is successful. Please continue next step by using 'core stats'.");
                        }
                    ...
                    // stats step
                    calModelStats();
                    log.info("Do model set statistics successfully. Please continue next step by using 'core varselect'.");
                } else if (args[0].equals(NORMALIZE_CMD)) {
                    // normalize step
                    normalizeTrainData();
                    log.info("Do model set normalization successfully. Please continue next step by using 'core train'.");
                } else if (args[0].equals(VARSELECT_CMD)) {
                    // variable selected step
                    selectModelVar();
                    log.info("Do model set variables selection successfully. Please continue next step by using 'core normalize'.");
                } else if (args[0].equals(TRAIN_CMD)) {
                    // train step
                    trainModel(cmd.hasOption(TRAIN_CMD_DRY), cmd.hasOption(TRAIN_CMD_DEBUG));
                    log.info("Do model set training successfully. Please continue next step by using 'core posttrain' or if no need posttrain you can go through with 'core eval'.");
                } else if (args[0].equals(POSTTRAIN_CMD)) {
                    // post train step
                    postTrainModel();
                    log.info("Do model set post-training successfully. Please configurate your eval set in ModelConfig.json and continue next step by using 'core eval' or 'core eval -new <eval set>' to create a new eval set.");
                    ...
                    if (args.length == 1) {
                        ...
                        log.info("Create eval set successfully. You can configurate EvalConfig.json or directly run 'core eval -run <evalSetName>' to get performance info.");
    ...
    private static void printModelSetCopiedSuccessfulLog(String newModelSetName) {
        ...
        log.info(String
                .format("Please change your folder to %s and then configurate your ModelConfig.json or dirctly do initilization step by 'core init.'",
                        newModelSetName));
    }
    ...
    private static void printUsage() {
        System.out.println("Usage: core COMMAND");

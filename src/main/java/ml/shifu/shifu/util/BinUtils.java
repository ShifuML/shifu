...
public final class BinUtils {
     ...
     * @return bin index, -1 if invalid values
     */
    public static int getNumericalBinIndexFloat(List<Float> binBoundaries, Object columnVal) {
        if(columnVal == null) {
            return -1;
        }
        ...
        if(columnVal instanceof Float) {
            dval = (Float) columnVal;
        } else if(columnVal instanceof Integer) {
            dval = ((Integer) columnVal).floatValue();
        } else {
            try {
                dval = Float.parseFloat(columnVal.toString());
            } catch (Exception e) {
                return -1;
            }
        }
     ...
     * @return bin index
     */
    public static int getBinIndex(List<Float> binBoundary, Float dVal) {
        assert binBoundary != null && binBoundary.size() > 0;
        assert dVal != null;
        int binSize = binBoundary.size();
        ...
        while(low <= high) {
            int mid = (low + high) >>> 1;
            Float midVal = binBoundary.get(mid);
            int cmp = midVal.compareTo(dVal);

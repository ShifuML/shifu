...
/**
 * {@link ParquetCorrelationMapper} is used to compute {@link CorrelationWritable} per column per mapper.
 * 
 * <p>
 ...
 */
public class ParquetCorrelationMapper extends Mapper<LongWritable, Tuple, IntWritable, CorrelationWritable> {

    private final static Logger LOG = LoggerFactory.getLogger(ParquetCorrelationMapper.class);
    ...
    @Override
    protected void map(LongWritable key, Tuple value, Context context) throws IOException, InterruptedException {
        ...
        double[] dValues = null;
        if(!this.dataPurifier.isFilter(value)) {
            return;
        }
        ...
        dValues = getDoubleArrayByRawArray(value);
    ...
    private double[] getDoubleArrayByRawArray(Tuple tuple) throws ExecException {
        ...
        for(int i = 0; i < columnConfigList.size(); i++) {
            ...
            if(columnConfig.getColumnFlag() == ColumnFlag.Meta) {
                ...
                if(this.tagSet.contains(tuple.get(i))) {
                    if(modelConfig.isRegression()) {
                        if(this.posTagSet.contains(tuple.get(i))) {
                            dValues[i] = 1d;
                        }
                        if(this.negTagSet.contains(tuple.get(i))) {
                            dValues[i] = 0d;
                        }
                    } else {
                        int index = -1;
                        for(int j = 0; j < tags.size(); j++) {
                            ...
                            if(tagSet.contains(tuple.get(0))) {
                                index = j;
                                break;
                            }
                        }
                ...
                if(columnConfig.isNumerical()) {
                    ...
                    if(tuple.get(i) == null || tuple.get(i).toString().length() == 0) {
                        // some null values, set it to min value to avoid parsing String to improve performance
                        dValues[i] = Double.MIN_VALUE;
                    } else {
                        dValues[i] = NumberFormatUtils.getDouble(tuple.get(i).toString(), Double.MIN_VALUE);
                    }
                ...
                if(columnConfig.isCategorical()) {
                    ...
                    Integer index = null;
                    if(tuple.get(i) != null) {
                        index = this.categoricalIndexMap.get(columnConfig.getColumnNum()).get(tuple.get(i));
                    }

...
public class DenseLayer extends AbstractLayer<float[], float[], float[], float[], DenseLayer>
    ...
    public float[] backward(float[] backInputs, float sig) {
        ...
        for(int i = 0; i < this.in; i++) {
            for(int j = 0; j < this.out; j++) {
                // basic derivatives
                this.wGrads[i][j] += (this.lastInput[i] * backInputs[j] * sig);
                // l2 loss derivatives
                this.wGrads[i][j] += (this.l2reg * this.weights[i][j] * sig);
            }
        }
        for(int j = 0; j < this.out; j++) {
            // no need l2 reg here as bias no need
            this.bGrads[j] = (backInputs[j] * sig);
        }
    ...
    public void update(DenseLayer gradLayer, Optimizer optimizer) {
        ...
        optimizer.update(this.bias, gradLayer.getbGrads(), getClass().getSimpleName());
    }
}

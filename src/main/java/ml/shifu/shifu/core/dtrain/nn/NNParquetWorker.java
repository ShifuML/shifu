...
public class NNParquetWorker extends AbstractNNWorker<Tuple> {
    ...
    public void load(GuaguaWritableAdapter<LongWritable> currentKey, GuaguaWritableAdapter<Tuple> currentValue,
        ...
        for (ColumnConfig columnConfig : this.columnConfigList) {
            float fval = DTrainUtils.parseRawNormValue(tuple, dataPos, 0.0f);
            ...
            if (columnConfig.isTarget()) {
                ...
                significance = DTrainUtils.parseRawNormValue(tuple, dataPos, 1.0f);
                ...
                if(subFeatureSet.contains(columnConfig.getColumnNum())) {
                    if(columnConfig.isMeta() || columnConfig.isForceRemove()) {
                        ...
                        for(int k = 0; k < columnConfig.getBinBoundary().size() + 1; k++) {
                            float tval = DTrainUtils.parseRawNormValue(tuple, dataPos, 0.0f);
        ...
        if (dataPos != tuple.size() -1 && dataPos != tuple.size()) {
            LOG.error("Normalization data has extra data. Expect {} or {}, actual is {}.", dataPos, dataPos + 1, tuple.size());
            throw new RuntimeException("Out of range Normalization data doesn't match with ColumnConfig.json.");
        }

        if (this.weightColumnId > 0 && !this.isWeightColumnMeta && dataPos == tuple.size() - 1) {
        ...
        } else if(this.weightColumnId > 0 && !this.isWeightColumnMeta && dataPos == tuple.size()) {

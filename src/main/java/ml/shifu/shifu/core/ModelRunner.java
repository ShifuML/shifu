 ...
 * <p/>
 * The output result for ModelRunnder is @CaseScoreResult. In the result, not only max/min/average
 * score will be stored, but also Map of raw input
 * <p/>
...
public class ModelRunner {
    ...
    public ModelRunner(
        ...
        this.scorer = new Scorer(models, columnConfigList, modelConfig.getAlgorithm(), modelConfig.getNormalizeStdDevCutOff());
    }
     ...
     * @param models           - models
     */
    public ModelRunner(List<ColumnConfig> columnConfigList, List<BasicML> models) {
        this(columnConfigList, models, Normalizer.STD_DEV_CUTOFF);
    }
    ...
    public ModelRunner(List<ColumnConfig> columnConfigList, List<BasicML> models, double stdDevCutoff) {
        this.columnConfigList = columnConfigList;
        this.scorer = new Scorer(models, columnConfigList,
                ALGORITHM.NN.name(),
                stdDevCutoff);
    }
     ...
     *
     * @param inputData - the whole original input data as String
     * @return @CaseScoreResult
     */
    public CaseScoreResult compute(String inputData) {
        if (dataDelimiter == null || header == null) {
            throw new UnsupportedOperationException("The dataDelimiter and header are null, please use right constructor!");
        }

        Map<String, String> rawDataMap =
                CommonUtils.convertDataIntoMap(inputData, dataDelimiter, header);
    ...
    public CaseScoreResult compute(Tuple tuple) throws ExecException {
        ...
        Map<String, String> rawDataMap =
                CommonUtils.convertDataIntoMap(tuple, header);

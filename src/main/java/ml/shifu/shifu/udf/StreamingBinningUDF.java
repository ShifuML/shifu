...
public class StreamingBinningUDF extends AbstractTrainerUDF<String>{

	private final static Logger log = LoggerFactory.getLogger(StreamingBinningUDF.class);
	...
	public StreamingBinningUDF(String source, String pathModelConfig,
			String pathColumnConfig) throws IOException {
		...
		// plus 1 for min/max value in bin
		estimator = new Estimator<Double>(maxBinSize + 1);
		...
		for (Tuple t : bag) {
			...
			case N:
				...
				try {
					...
					log.warn(
							"Fail to handle column: {} {}  expected is numerical, but could not cast to double",
    ...
    private String listToString(List<Double> input) {

        StringBuilder sb = new StringBuilder();

        for (Double d : input){
            sb.append(d.toString() + CATEGORY_VAL_SEPARATOR);
        }
    ...
    private String setToString(Set<String> input) {
        ...
        for (String d : input){
            sb.append(d + CATEGORY_VAL_SEPARATOR);
        }
	...
	public String getValue() {
		...
		if (thisColumn.getColumnType().equals(ColumnConfig.ColumnType.N)
				&& !modelConfig.getBinningMethod().equals(
						ModelStatsConf.BinningMethod.EqualInterval)) {
			// numeric binning
